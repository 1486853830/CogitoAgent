...
- [x] **R2.7 推测执行（Speculative Execution）**：主模型推理期间并行跑轻量"草稿"模型预测下一步工具调用，仅对只读/幂等工具预执行（`classifyTool` 基于 R1.7 注解）；预测命中则复用推测结果，未命中无损回退到标准顺序路径；默认关闭，`chat.speculative.enabled` 或 `auto` 开启
- [x] **R2.8 PASTE 模式挖掘**：工具分类（可推测 / dry-run 可行 / 禁止推测）基于 R1.7 注解；`PatternStore` 从执行轨迹挖掘"模式元组"（上下文签名 → 预测工具），记录命中率；`auto` 模式下命中率与样本数双达标后自动启用推测执行
- [x] **R2.9 推理深度可调旋钮**：`reasoning_effort` 补齐 `xhigh`（原 none 保留作禁用）；新增 `verbosity`（low/medium/high）与 `thinking`（adaptive / enabled+budgetTokens / disabled）并端到端打通 config→Agent→client 请求体
...
- [x] **R3.5 JIT 上下文策略**：从"预推理嵌入检索"升级为即时上下文加载——JIT 标记注入（`[上下文补充]` 标记随对话持久化），跨上下文窗口的工具数据通过 `stashJitToolData`/`getJitToolData` 按需存取（内存 Map + id 引用，避免整块输出驻留对话），`compressHistory` 与分区摘要均识别 JIT 标记
- [x] **R3.6 结构化笔记**：Agent 在工作过程中维护结构化笔记（关键发现、中间结果、待办事项），作为跨上下文窗口的持久化知识载体，压缩时笔记优先保留
- [x] **R3.7 上下文腐烂防御**：针对 Transformer 长序列注意力"拉薄"问题，实施上下文分区策略——`getPartitionedMessages()` 将历史划分为 system / workingMemory / dialogue 三段（识别并路由 `[上下文补充]` JIT 标记归入 workingMemory），`summarizePartitions()` 为各分区独立生成紧凑摘要（支持注入 LLM 摘要器，失败回退朴素摘要），`getPartitionStats()` 提供分区级 token 统计供分层压缩触发
...
- [x] **R4.5 MCP Elicitation**：MCP 服务端可通过客户端向用户请求结构化输入，实现更丰富的人机交互。实现（客户端侧，R4.5/R4.6 能力接线）：`config.mcp.elicitation` 开启后，外部 client 声明 elicitation capability 并注册 `elicitation/create` 请求处理器（`mcp.ts` registerAdvanceClientHandlers），把外部 server 的结构化输入请求桥接到本地确认通道（`requestConfirmation` 呈现 message + requestedSchema），用户批准返回 accept、拒绝返回 decline，无人值守时 fail-closed 拒绝；实现可注入（`setAdvanceInteractionHandlers`）
- [x] **R4.6 MCP Sampling**：MCP 服务端可从客户端请求 LLM 补全（含工具调用），使服务端从被动工具提供者升级为可编排多步推理流的协调者。实现（客户端侧）：`config.mcp.sampling` 开启后声明 sampling capability 并注册 `sampling/createMessage` 请求处理器，把外部 server 的 LLM 补全请求（messages/maxTokens/temperature）转发给本地 `chatText` 完成真实采样，返回 `{ model, role: 'assistant', content: { type: 'text', text } }`；文本块归一化处理非文本输入；可注入自定义实现（`setAdvanceInteractionHandlers`）
- [x] **R4.7 MCP 工具注解传播**：将 R1.7 的工具注解通过 MCP `toolAnnotations` 字段传播给外部客户端，实现跨客户端的统一策略执行
- [x] **R4.8 MCP 无状态重构对齐**：对齐 2026-07 MCP 重大重构——取消协议层 Session，改为每次请求携带完整处理信息。实现：外部 server 注册表支持 `stateless` 标志（`mcp.ts` getExternalSessionInfo）；stateless 模式下 `callTool` 每次请求注入 `_meta.sessionId`（`generateExternalSessionId`），并从响应 `_session`/`_meta` 回读 `readResponseSessionId` 供后续无状态续接，支持 MCP Server 在 Kubernetes 上水平扩缩容

### R5. 插件体系

- [x] **R5.1 插件工具注入**：`plugin.ts` 动态注册的工具自动生成 JSON Schema 并注入系统提示 / 原生 tools，解决"插件工具模型不可见"
- [x] **R5.2 插件权限模型**：插件在 manifest `metadata.defaultPermission` 声明默认权限级别（`allow`/`ask`/`deny`，模板已含示例）；`resolveToolPermission` 解析优先级为「全局 `tools.permissions` 规则 > 插件默认 > allow」；Dashboard 扩展面板可编辑 `tools.permissions` 规则集（IPC `get-extensions`/`set-tool-permissions` 全量替换）。**执行期强制生效**：门禁已接入两条（也是仅有的两条）工具执行路径——① 主 Agent `Agent.ts executeTool`（原生工具调用 / 推测执行 / 插件 / MCP 工具最终都汇聚于此），`deny` 直接拒绝、`ask` 走一次用户授权并与危险操作确认合并为单次弹窗；② 子智能体 `orchestrator.ts`（直接调 `registry.fn`，绕过 executeTool，故独立校验），因无交互通道 `ask` 与 `deny` 一并拒绝。此外推测执行（R2.7）仅对 `allow` 工具预执行，避免猜测阶段弹授权框或提前执行受限工具。注：权限声明落地为插件 `metadata`（与现有 `export const metadata` 约定一致），而非独立的 `plugin.yaml`
  - ⚠️ **已知限制**：子智能体路径的 `ask` 被降级为拒绝（fail-closed）。子智能体运行在无 UI 交互通道的上下文中，无法向用户发起授权询问，因此保守拒绝而非静默放行。副作用是「主 Agent 能经用户授权执行、子智能体一律不能执行」的行为不一致——对 `ask` 级工具，委派给子智能体会直接失败。解决需要跨进程审批链路，见 **R5.9**
- [x] **R5.3 插件热加载**：运行中加载 / 卸载插件（不重启 Agent），UI 面板反映
- [x] **R5.4 插件安全**：不可信插件默认受限运行；文档明确插件执行权限边界。实现：① 信任模型——`isPluginTrusted(name)` 判定插件是否受信任，受信任来源为 `config.security.trustedPlugins` 白名单列表或环境变量 `COGITO_TRUSTED_PLUGINS`（逗号分隔，显式用户覆盖），未列入的插件一律视为不可信（fail-closed）；② 默认受限——`getUntrustedPluginPermission()` 返回不可信插件的默认权限级别（默认 `ask`，可在 `config.security.untrustedPluginPermission` 配置为 `allow`/`deny`/`ask`），不可信插件的工具在无全局规则时落到该受限级别，且其 manifest 声明的 `defaultPermission` 不被信任、仅对受信任插件生效（防止不可信代码自我授权）；③ `resolveToolPermission` 扩展为三级 `globalLevel > pluginDefault > untrustedFallback`，保持 R5.2 的全局规则最高优先级与纯函数可测性；④ 权限边界文档化——全局 `tools.permissions` 规则是最终裁决（对主 Agent `Agent.ts executeTool`、子智能体 `orchestrator.ts`、MCP 通道 `mcp.ts` 三条执行路径一律生效），插件 manifest 声明仅是受信任插件缺省值，用户可随时用 Dashboard 覆盖。"插件能干什么"由用户显式配置决定，插件自身不能提高自己的权限
...
- [ ] **R5.9 子智能体授权冒泡（Permission Escalation Chain）**：解除 R5.2 的已知限制——子智能体遇到 `ask` 级工具时，将授权请求冒泡到主 Agent 由用户裁决，而非直接拒绝。需要：① 子智能体侧挂起工具调用并上报待批请求（含 agent id / 工具名 / 参数摘要 / 用途说明）；② 主 Agent 侧汇聚多子智能体的并发请求并串行呈现（避免弹窗风暴）；③ 授权结果回传并唤醒挂起的子智能体，含超时兜底（超时按 deny 处理，保持 fail-closed）；④ 审批粒度选项（本次 / 本任务内该工具 / 永久写入 `tools.permissions`）。安全约束：授权上下文必须展示发起方是子智能体而非用户直接请求，防止子智能体借授权链路提权
...
- [~] **R7.3 真并行**：`panelDiscussion` / `voting` 已改为真并行（`Promise.allSettled`，orchestrator.ts），`pipeline` 失败补偿仍未实现（目前 step 失败即中止）
...
| R2.7   | 推测执行                | [x]  | v2.4.1   | 2026-08-08 | 草稿模型并行预测 + 仅只读/幂等工具预执行 + 命中复用/未命中无损回退；classifyTool 基于 R1.7 注解；默认关闭                                                                                                                                         |
| R2.8   | PASTE 模式挖掘          | [x]  | v2.4.1   | 2026-08-08 | PatternStore 文件持久化模式元组；命中率+样本数双门槛自动启用推测；工具三分类（可推测/dry-run/禁止）                                                                                                                                               |
| R2.9   | 推理深度可调旋钮        | [x]  | v2.4.1   | 2026-08-08 | reasoning_effort 补 xhigh；新增 verbosity 与 thinking(adaptive/enabled/disabled)，经 client 请求体打通                                                                                                                                            |
...
| R3.5   | JIT 上下文策略          | [x]  | v2.4.1   | 2026-08-08 | JIT 标记注入（[上下文补充] 随对话持久化）；stashJitToolData/getJitToolData 按需存取工具数据；压缩/分区识别 JIT 标记                                                                                                                               |
| R3.6   | 结构化笔记              | [x]  | v2.4.0   | 2026-08-08 | saveSessionNote / listSessionNotes                                                                                                                                                                                                                |
| R3.7   | 上下文腐烂防御          | [x]  | —        | —          | getPartitionedMessages 三分区 + summarizePartitions 每区独立摘要 + getPartitionStats 分区 token 统计                                                                                                                                              |
| R3.8   | 多 Agent 隔离上下文     | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R3.9   | 会话中途系统消息        | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
...
| R4.5   | MCP Elicitation         | [x]  | —        | —          | mcp.elicitation 开启 → 声明 capability + 注册 elicitation/create 处理器桥接本地确认通道（fail-closed）；可注入 setAdvanceInteractionHandlers                                                                                                      |
| R4.6   | MCP Sampling            | [x]  | —        | —          | mcp.sampling 开启 → 声明 capability + 注册 sampling/createMessage 转发本地 chatText 真实采样；可注入实现                                                                                                                                          |
| R4.7   | MCP 工具注解传播        | [x]  | v2.4.0   | 2026-08-08 | annotations → MCP toolAnnotations                                                                                                                                                                                                                 |
| R4.8   | MCP 无状态重构对齐      | [x]  | —        | —          | stateless 标志 + 每次 callTool 注入/回读 _meta.sessionId（generateExternalSessionId / readResponseSessionId）                                                                                                                                     |
| R5.1   | 插件工具注入            | [x]  | v2.4.0   | 2026-08-08 | schema/param/annotations 随注册注入                                                                                                                                                                                                               |
| R5.2   | 插件权限模型            | [x]  | —        | —          | metadata.defaultPermission 回退 + resolveToolPermission + Dashboard 规则编辑；门禁已接入 executeTool 与 orchestrator 两条执行路径（deny 拦截 / ask 授权），推测执行仅限 allow。⚠️ 已知限制：子智能体侧 ask 降级为拒绝（fail-closed），解除见 R5.9 |
| R5.3   | 插件热加载              | [x]  | v2.4.0   | 2026-08-08 | unloadPlugin + reloadPlugins                                                                                                                                                                                                                      |
| R5.4   | 插件安全                | [x]  | —        | —          | isPluginTrusted（security.trustedPlugins / COGITO_TRUSTED_PLUGINS）+ 不可信插件默认受限（untrustedPluginPermission 默认 ask，manifest 自声明权限不受信）+ resolveToolPermission 三级解析 + 三条执行路径权限边界文档化                             |
| R5.5   | Agent Skills 标准       | [x]  | v2.4.0   | 2026-08-08 | SKILL.md front-matter 扫描                                                                                                                                                                                                                        |
| R5.6   | Skills 目录约定         | [~]  | —        | —          | skills/ 目录注入；.cogito/skills 约定待统一                                                                                                                                                                                                       |
| R5.7   | Code Mode 工具范式      | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R5.8   | 配置坏味道防御          | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R5.9   | 子智能体授权冒泡        | [ ]  | —        | —          | 解除 R5.2 已知限制：子智能体 ask 请求冒泡至主 Agent 由用户裁决，含并发汇聚 / 超时按 deny 兜底 / 审批粒度                                                                                                                                          |
| R6.1   | 向量记忆                | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R6.2   | 分级记忆                | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R6.3   | 记忆自动写入            | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R6.4   | 记忆持久化              | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
...
| R7.1   | 统一 runner             | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R7.2   | agent 线程管理          | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R7.3   | 真并行                  | [~]  | —        | —          | panelDiscussion/voting 已 allSettled 真并行；pipeline 失败补偿未做（step 失败即中止）                                                                                                                                                             |
| R7.4   | 通道抽象打通            | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R7.5   | 子代理安全              | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R7.6   | A2A 协议集成            | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |
| R7.7   | 嵌套子 Agent            | [ ]  | —        | —          |                                                                                                                                                                                                                                                   |

...
(function () {
  ...
  const MESSAGES = {
    ...
    'dashboard.ext.title': {
      zh: '扩展配置',
      en: 'Extensions',
      es: 'Configuración de extensiones',
      hu: 'Bővítmények beállítása',
      ru: 'Настройки расширений',
    },
    'dashboard.ext.subtitle': {
      zh: '管理 MCP 服务器与插件工具权限',
      en: 'Manage MCP servers and plugin tool permissions',
      es: 'Gestionar servidores MCP y permisos de complementos',
      hu: 'MCP szerverek és bővítményeszköz-jogosultságok kezelése',
      ru: 'Управление MCP-серверами и правами плагинов',
    },
    'dashboard.ext.mcpServers': {
      zh: 'MCP 服务器',
      en: 'MCP Servers',
      es: 'Servidores MCP',
      hu: 'MCP szerverek',
    ...
    'dashboard.ext.mcpServerName': {
      zh: '本地 Server 名称（供外部客户端连接）',
      en: 'Local server name (for external clients)',
      es: 'Nombre del servidor local (para clientes externos)',
      hu: 'Helyi szerver neve (külső klienseknek)',
      ru: 'Имя локального сервера (для внешних клиентов)',
    },
    'dashboard.ext.mcpServerNamePh': {
      zh: 'cogito-agent',
      en: 'cogito-agent',
      es: 'cogito-agent',
      hu: 'cogito-agent',
    ...
    'dashboard.ext.mcpPrefix': {
      zh: '外部 Server 工具前缀',
      en: 'External server tool prefix',
      es: 'Prefijo de herramientas del servidor externo',
      hu: 'Külső szerver eszköz-előtagja',
      ru: 'Префикс инструментов внешнего сервера',
    },
    'dashboard.ext.mcpPrefixPh': {
      zh: 'mcp_',
      en: 'mcp_',
      es: 'mcp_',
      hu: 'mcp_',
    ...
    'dashboard.ext.mcpRegistry': {
      zh: '外部 MCP Server 注册表',
      en: 'External MCP server registry',
      es: 'Registro de servidores MCP externos',
      hu: 'Külső MCP szerver-regisztráció',
      ru: 'Реестр внешних MCP-серверов',
    },
    'dashboard.ext.addServer': {
      zh: '+ 添加服务器',
      en: '+ Add server',
      es: '+ Añadir servidor',
      hu: '+ Szerver hozzáadása',
      ru: '+ Добавить сервер',
    },
    'dashboard.ext.mcpHint': {
      zh: '配置保存后需重启应用（或重新配置）以让 Agent 加载新的 MCP 工具。',
      en: 'Restart the app (or reconfigure) after saving so the Agent loads the new MCP tools.',
      es: 'Reinicie la aplicación (o reconfigure) tras guardar para que el Agent cargue las nuevas herramientas MCP.',
      hu: 'A mentés után indítsa újra az alkalmazást (vagy állítsa be újra), hogy az Agent betöltse az új MCP eszközöket.',
      ru: 'После сохранения перезапустите приложение (или перенастройте), чтобы Agent загрузил новые MCP-инструменты.',
    },
    'dashboard.ext.plugins': {
      zh: '已安装插件',
      en: 'Installed plugins',
      es: 'Complementos instalados',
      hu: 'Telepített bővítmények',
      ru: 'Установленные плагины',
    },
    'dashboard.ext.permRules': {
      zh: '工具权限规则',
      en: 'Tool permission rules',
      es: 'Reglas de permisos de herramientas',
      hu: 'Eszközjogosultsági szabályok',
      ru: 'Правила прав доступа инструментов',
    },
    'dashboard.ext.addRule': {
      zh: '+ 添加规则',
      en: '+ Add rule',
      es: '+ Añadir regla',
    ...
    'dashboard.ext.permHint': {
      zh: '规则优先级：显式规则 > 插件声明默认值 > 允许。deny 将禁用该工具。',
      en: 'Priority: explicit rule > plugin default > allow. deny disables the tool.',
      es: 'Prioridad: regla explícita > predeterminado del complemento > permitir. deny desactiva la herramienta.',
      hu: 'Prioritás: explicit szabály > bővítmény alapértelmezés > engedélyezés. deny letiltja az eszközt.',
      ru: 'Приоритет: явное правило > умолчание плагина > разрешить. deny отключает инструмент.',
    },
    'dashboard.ext.save': {
      zh: '保存',
      en: 'Save',
    ...
    'dashboard.ext.noPlugins': {
      zh: '未检测到插件（plugins/ 目录下无 index.js）',
      en: 'No plugins detected (no index.js in plugins/)',
      es: 'No se detectaron complementos (sin index.js en plugins/)',
      hu: 'Nincsenek bővítmények (nincs index.js a plugins/ alatt)',
      ru: 'Плагины не обнаружены (нет index.js в plugins/)',
    },
    'dashboard.ext.noServers': {
      zh: '暂无外部 MCP 服务器',
      en: 'No external MCP servers yet',
      es: 'Aún no hay servidores MCP externos',
      hu: 'Még nincsenek külső MCP szerverek',
      ru: 'Пока нет внешних MCP-серверов',
    },
    'dashboard.ext.cmd': {
      zh: '命令',
      en: 'Command',
    ...
    'dashboard.ext.args': {
      ...
      hu: 'Argumentumok (szóközzel elválasztva)',
      ru: 'Аргументы (через пробел)',
    },
    'dashboard.ext.env': {
      zh: '环境变量（每行 KEY=VALUE）',
      en: 'Environment variables (one KEY=VALUE per line)',
      es: 'Variables de entorno (una KEY=VALUE por línea)',
      hu: 'Környezeti változók (egy KEY=VALUE soronként)',
      ru: 'Переменные среды (одна KEY=VALUE на строку)',
    },
    'dashboard.ext.delete': {
      zh: '删除',
      en: 'Delete',
      es: 'Eliminar',
      hu: 'Törlés',
    ...
    'dashboard.ext.noRules': {
      zh: '暂无权限规则',
      en: 'No permission rules yet',
      es: 'Aún no hay reglas de permisos',
      hu: 'Még nincsenek jogosultsági szabályok',
      ru: 'Пока нет правил прав доступа',
    },
    'dashboard.ext.toolNamePh': {
      zh: '工具名',
      en: 'Tool name',
    ...
    'setup.descAdvanced': {
      zh: '配置启动模式、邮件服务、OCR、视觉分析、图像生成、科学计算等',
      en: 'Configure startup mode, email, OCR, vision, image generation, and science',
      es: 'Configura modo de inicio, email, OCR, visión, generación de imágenes, etc.',
      hu: 'Konfigurálja az indítómódot, e-mailt, OCR-t, látást, képgenerálást, stb.',
      ru: 'Настройте режим запуска, почту, OCR, зрительный анализ, генерацию изображений и др.',
    },
    'setup.modeLabel': {
      zh: '启动模式',
      en: 'Startup mode',
    ...
    'setup.imageGenKeyHint': {
      zh: '不填则使用主 API Key（与语言模型相同）',
      en: 'Uses the main API Key if empty (same as the language model)',
      es: 'Usa la clave principal si está vacío (igual que el modelo de lenguaje)',
      hu: 'Üresen a fő API-kulcsot használja (azonos a nyelvi modellel)',
      ru: 'При пустом поле используется основной ключ (тот же, что у языковой модели)',
    },
    'setup.imageGenBasePh': {
      zh: 'ImageGen API 地址',
      en: 'ImageGen API address',
      es: 'Dirección de la API ImageGen',
      hu: 'ImageGen API címe',
      ru: 'Адрес ImageGen API',
    },
    'setup.imageGenBaseHint': {
      zh: '例如: https://api.moark.com/v1，不填则使用默认地址',
      en: 'e.g. https://api.moark.com/v1; uses the default if empty',
      es: 'p. ej. https://api.moark.com/v1; usa la predeterminada si está vacío',
      hu: 'pl. https://api.moark.com/v1; üresen az alapértelmezettet használja',
      ru: 'Например https://api.moark.com/v1; используется адрес по умолчанию при пустом',
    },
    'setup.imageGenModelHint': {
      zh: '图像生成模型名称',
      en: 'Image generation model name',
      es: 'Nombre del modelo de generación',
      hu: 'Képgenerálási modell neve',
      ru: 'Название модели генерации изображений',
    },
    'setup.searchSection': {
      zh: '联网搜索',
      en: 'Web Search',
    ...
    'setup.searchEnabledLabel': {
      zh: '启用联网搜索',
      en: 'Enable web search',
      es: 'Habilitar búsqueda web',
      hu: 'Webes keresés bekapcsolása',
      ru: 'Включить веб-поиск',
    },
    'setup.searchEnabledHint': {
      zh: '关闭后搜索工具将直接返回"未启用"',
      en: 'When disabled, the search tool returns "not enabled"',
      es: 'Si está desactivado, la herramienta de búsqueda devuelve "no activado"',
      hu: 'Kikapcsolva a keresőeszköz "nincs engedélyezve" üzenetet ad',
      ru: 'Если выключено, инструмент поиска вернёт «не включено»',
    },
    'setup.searchBaseURLPh': {
      zh: '搜索 API 地址（moark 可留空）',
      en: 'Search API address (leave empty for moark)',
      es: 'Dirección de la API de búsqueda (vacío para moark)',
      hu: 'Kereső API címe (moark esetén üresen hagyható)',
      ru: 'Адрес API поиска (для moark можно оставить пустым)',
    },
    'setup.searchBaseURLHint': {
      zh: 'moark 供应商可留空自动推导；其他供应商需填写搜索端点',
      en: 'Leave empty for moark (auto-derived); required for other providers',
      es: 'Vacío para moark (derivado automáticamente); obligatorio para otros proveedores',
      hu: 'Moark esetén üresen hagyható (automatikusan levezetve); más szolgáltatóknál kötelező',
      ru: 'Для moark можно оставить пустым (выводится автоматически); для других провайдеров обязательно',
    },
    'setup.searchRecencyHint': {
      zh: '搜索时效过滤（如 week / month），留空为不过滤',
      en: 'Recency filter (e.g. week / month); empty = no filter',
      es: 'Filtro de actualidad (p. ej. week / month); vacío = sin filtro',
      hu: 'Frissességi szűrő (pl. week / month); üres = nincs szűrés',
      ru: 'Фильтр свежести (например week / month); пусто = без фильтра',
    },
    'setup.searchSiteHint': {
      zh: '搜索站点过滤，留空为不限制',
      en: 'Site filter (comma separated); empty = no restriction',
      es: 'Filtro de sitios (separados por comas); vacío = sin restricción',
      hu: 'Oldalszűrő (vesszővel elválasztva); üres = nincs korlátozás',
      ru: 'Фильтр сайтов (через запятую); пусто = без ограничений',
    },
    'setup.codeSection': {
      zh: '代码执行',
      en: 'Code Execution',

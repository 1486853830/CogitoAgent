...
export function buildEnvLines(config) {
  ...
  return [
    ...
    '# ============================================',
    '',
    '# 图像生成 API 密钥（可选，默认回退到 COGITO_API_KEY / COGITO_MOARK_API_KEY）',
    `COGITO_IMAGEGEN_API_KEY=${q(c.imageGen?.apiKey)}`,
    '',
    '# 图像生成 API 服务地址（可选，默认 https://api.moark.com/v1）',
    `COGITO_IMAGEGEN_API_BASE_URL=${q(c.imageGen?.baseURL)}`,
    '',
    '# 图像生成模型名称（可选，默认 qwen-image-2.0-pro）',
    `COGITO_IMAGEGEN_MODEL=${q(c.imageGen?.model || 'qwen-image-2.0-pro')}`,
    '',
    '# ============================================',
    '# 代码执行配置（可选）',
    '# ============================================',

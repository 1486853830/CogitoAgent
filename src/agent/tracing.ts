...
class TraceRecord {
  ...
  constructor(event: string, data: TraceData) {
    this.id = generateId();
 ...
 * 初始化追踪会话
 */
function initTracing(sessionId?: string): string {
 ...
 * 记录追踪事件
 */
function log(event: string, data: TraceData = {}): TraceRecord | undefined {
  ...
  if (traces.length > MAX_TRACES) {
    traces = traces.slice(-Math.floor(MAX_TRACES / 2));
  }
 ...
 * 追踪 LLM 调用
 */
function traceLLMCall(model: string, inputLength: number, outputLength: number, duration: number, tokensUsed: any = null): TraceRecord | undefined {
 ...
 * 追踪工具执行
 */
function traceToolExec(toolName: string, args: any, result: any, duration: number, success: boolean): TraceRecord | undefined {
 ...
 * 追踪状态转换
 */
function traceStateChange(fromState: string, toState: string, reason: string | null = null): TraceRecord | undefined {
 ...
 * 追踪错误
 */
function traceError(error: Error, context: Record<string, any> = {}): TraceRecord | undefined {
 ...
 * 启用/禁用追踪
 */
function setTracingEnabled(enabled: boolean): void {
 ...
 * 获取最近的追踪记录
 */
function getRecentTraces(count = 50): TraceRecord[] {
 ...
 * 创建计时器
 */
function startTimer(): { start: number; end: number | null; duration: number; stop(): number } {
  return {
    start: Date.now(),
    end: null,
    get duration(): number {

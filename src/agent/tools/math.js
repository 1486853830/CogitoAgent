...
async function describe(data) {
  try {
    ...
    const modes = Object.entries(freq).filter(([, c]) => c === maxFreq).map(([k]) => parseFloat(k));
    ...
    let skewness = 0;
    if (std > 0) {
      skewness = values.reduce((s, v) => s + Math.pow((v - mean) / std, 3), 0) * n / ((n - 1) * (n - 2));
    }
 ...
 * @returns {Promise<Object>}
 */
async function correlation(x, y) {
  try {
...
async function linearRegression(x, y) {
  try {
    ...
    // R²
    const ssRes = yArr.reduce((s, v, i) => s + (v - (a * xArr[i] + b)) ** 2, 0);
 ...
 * @returns {Promise<Object>}
 */
async function matrixDeterminant(A) {
  try {
    const mA = typeof A === 'string' ? JSON.parse(A) : A;
    const n = mA.length;
 ...
 * @returns {Promise<Object>}
 */
async function matrixInverse(A) {
  try {
    const mA = typeof A === 'string' ? JSON.parse(A) : A;
    const n = mA.length;
    ...
    // 格式化
    const formatted = inv.map(r => r.map(v => parseFloat(v.toFixed(4))));
 ...
 * @returns {Promise<Object>}
 */
async function solveQuadratic(a, b, c) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function combination(n, k) {
  try {
    n = parseInt(n);
    k = parseInt(k);
    if (isNaN(n) || n < 0) return { success: false, error: 'n 必须为非负整数' };
 ...
 * @returns {Promise<Object>}
 */
async function permutation(n, k) {
  try {
    n = parseInt(n);
    k = parseInt(k);
    if (isNaN(n) || n < 0) return { success: false, error: 'n 必须为非负整数' };
 ...
 * @returns {Promise<Object>}
 */
async function siConvert(value, from, to) {
  try {

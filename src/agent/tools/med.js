 ...
 * @returns {Promise<Object>}
 */
async function bsa(weight, height, formula = 'mosteller') {
  try {
    if (!weight || !height || weight <= 0 || height <= 0) {
      return { success: false, error: '体重和身高必须为正数' };
    }
 ...
 * @returns {Promise<Object>}
 */
async function egfr(creatinine, age, gender) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function crcl(creatinine, age, weight, gender) {
  try {
    if (!creatinine || creatinine <= 0) return { success: false, error: '肌酐值必须为正数' };
    if (!age || age <= 0 || age > 150) return { success: false, error: '年龄无效' };
 ...
 * @returns {Promise<Object>}
 */
async function childPugh(bilirubin, albumin, inr, ascites, encephalopathy) {
  try {
    const points = {};
 ...
 * @returns {Promise<Object>}
 */
async function calculateDose(weight, dosePerKg, unit = 'mg') {
  try {
    if (!weight || weight <= 0) return { success: false, error: '体重必须为正数' };
 ...
 * @returns {Promise<Object>}
 */
async function bsaDose(bsaValue, dosePerM2, unit = 'mg') {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function infusionRate(volume, time, timeUnit = 'h') {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function idealBodyWeight(height, gender) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function convertUnit(value, from, to) {
  try {
    ...
    const conversions = [
      // 葡萄糖 mg/dL ↔ mmol/L
      { test: (f, t) => (f.includes('mg/dl') || f === 'mg%') && (t === 'mmol/l' || t === 'mmoll'), calc: (v) => v * 0.0555, label: '葡萄糖' },
      { test: (f, t) => (f === 'mmol/l' || f === 'mmoll') && (t.includes('mg/dl') || t === 'mg%'), calc: (v) => v / 0.0555, label: '葡萄糖' },
      // 肌酐 mg/dL ↔ μmol/L
      { test: (f, t) => f.includes('mg/dl') && (t.includes('μmol/l') || t.includes('umol/l')), calc: (v) => v * 88.4, label: '肌酐' },
      { test: (f, t) => (f.includes('μmol/l') || f.includes('umol/l')) && t.includes('mg/dl'), calc: (v) => v / 88.4, label: '肌酐' },
      // 尿素氮 mg/dL ↔ mmol/L
      { test: (f, t) => f.includes('mg/dl') && t.includes('mmol/l'), calc: (v) => v * 0.357, label: '尿素氮' },
      { test: (f, t) => f.includes('mmol/l') && t.includes('mg/dl'), calc: (v) => v / 0.357, label: '尿素氮' },
      // 胆红素 mg/dL ↔ μmol/L
      { test: (f, t) => f.includes('mg/dl') && (t.includes('μmol/l') || t.includes('umol/l')), calc: (v) => v * 17.1, label: '胆红素' },
      { test: (f, t) => (f.includes('μmol/l') || f.includes('umol/l')) && t.includes('mg/dl'), calc: (v) => v / 17.1, label: '胆红素' },
      // 钙 mg/dL ↔ mmol/L
      { test: (f, t) => f.includes('mg/dl') && t.includes('mmol/l'), calc: (v) => v * 0.2495, label: '钙' },
      { test: (f, t) => f.includes('mmol/l') && t.includes('mg/dl'), calc: (v) => v / 0.2495, label: '钙' },
      // 胆固醇 mg/dL ↔ mmol/L
      { test: (f, t) => f.includes('mg/dl') && t.includes('mmol/l'), calc: (v) => v * 0.0259, label: '胆固醇' },
      { test: (f, t) => f.includes('mmol/l') && t.includes('mg/dl'), calc: (v) => v / 0.0259, label: '胆固醇' },
      // 甘油三酯 mg/dL ↔ mmol/L
      { test: (f, t) => f.includes('mg/dl') && t.includes('mmol/l'), calc: (v) => v * 0.0113, label: '甘油三酯' },
      { test: (f, t) => f.includes('mmol/l') && t.includes('mg/dl'), calc: (v) => v / 0.0113, label: '甘油三酯' },
      // 血压 mmHg ↔ kPa
      { test: (f, t) => (f.includes('mmhg') || f === 'mmhg') && (t.includes('kpa') || t === 'kpa'), calc: (v) => v * 0.1333, label: '血压' },
      { test: (f, t) => (f.includes('kpa') || f === 'kpa') && (t.includes('mmhg') || t === 'mmhg'), calc: (v) => v / 0.1333, label: '血压' },
    ];
 ...
 * @returns {Promise<Object>}
 */
async function temperatureConvert(value, from, to) {
  try {
    if (value === undefined || value === null) {
 ...
 * @returns {Promise<Object>}
 */
async function meanArterialPressure(sbp, dbp) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function anionGap(na, cl, hco3) {
  try {
    if (!na || !cl || !hco3) {
 ...
 * @returns {Promise<Object>}
 */
async function correctedCalcium(calcium, albumin) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function oxygenIndex(pao2, fio2) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function parseVitalSigns(filePath) {
 ...
 * @returns {Promise<Object>}
 */
async function vitalsReport(filePath) {
  const basePath = getBasePath();
  const fullPath = path.isAbsolute(filePath) ? filePath : path.join(basePath, filePath);

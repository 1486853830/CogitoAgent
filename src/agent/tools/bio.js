...
// DNA 互补碱基映射
const DNA_COMPLEMENT = { 'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C', 'a': 't', 't': 'a', 'c': 'g', 'g': 'c' };
...
// DNA 碱基分子量（Da）
const DNA_BASE_MW = { 'A': 313.21, 'T': 304.20, 'C': 289.18, 'G': 329.21 };
 ...
 * 清理序列（去除非序列字符）
 */
function cleanSeq(seq, validChars) {
 ...
 * 验证序列是否有效
 */
function validateSeq(seq, validChars, name) {
  if (!seq || seq.length === 0) {
 ...
 */
async function dnaComplement(seq) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function dnaReverseComplement(seq) {
  try {
    const valid = validateSeq(seq, DNA_CHARS, 'DNA');
    if (!valid.success) return valid;
    const cleaned = valid.data;
 ...
 * @returns {Promise<Object>}
 */
async function rnaTranscribe(seq) {
  try {
    const valid = validateSeq(seq, DNA_CHARS, 'DNA');
    if (!valid.success) return valid;
    const cleaned = valid.data;
 ...
 * @returns {Promise<Object>}
 */
async function translate(seq, readingFrame = 0) {
  try {
    ...
    const aaNames = codons.map(c => `${AA_THREE_LETTER[c.aa] || '?'}`).join('-');
 ...
 * @returns {Promise<Object>}
 */
async function molecularWeight(seq) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function hammingDistance(seq1, seq2) {
  try {
    if (!seq1 || !seq2) {
 ...
 * @returns {Promise<Object>}
 */
async function levenshteinDistance(seq1, seq2) {
  try {
    if (!seq1 || !seq2) {
      return { success: false, error: '两个序列都不能为空' };
    }
 ...
 * @returns {Promise<Object>}
 */
async function hairpinCheck(seq) {
  try {
    const valid = validateSeq(seq, DNA_CHARS, 'DNA');
    if (!valid.success) return valid;
    const cleaned = valid.data;
 ...
 * @returns {Promise<Object>}
 */
async function parseFASTQ(filePath) {
  const basePath = getBasePath();
  const fullPath = path.isAbsolute(filePath) ? filePath : path.join(basePath, filePath);
 ...
 * @returns {Promise<Object>}
 */
async function fastaToCSV(fastaPath, csvPath) {
  const basePath = getBasePath();
 ...
 * @returns {Promise<Object>}
 */
async function randomSeq(length, type = 'dna') {
  try {

...
async function wechatApiPost(endpoint, body) {
...
async function startWechatPolling(handler) {
  await loadState();
...
async function stopWechatPolling() {
  pollingActive = false;
...
async function sendWechatMessage(to, text) {
  await loadState();
...
async function sendWechatImage(to, imagePath) {
  await loadState();
...
async function generateWechatQRCode() {
  try {
    ...
    wechatState.sessionKey = result.sessionKey;

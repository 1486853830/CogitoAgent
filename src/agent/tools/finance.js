 ...
 */
async function compoundInterest(P, r, n, t) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function presentValue(FV, r, n) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function futureValueAnnuity(PMT, r, n) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function npv(rate, cashflows) {
  try {
    rate = toNum(rate);
    ...
    const totalInvestment = Math.abs(flows.filter(f => f < 0).reduce((s, v) => s + v, 0));
    return {
      success: true,
 ...
 * @returns {Promise<Object>}
 */
async function paybackPeriod(cashflows) {
  try {
    const flows = typeof cashflows === 'string' ? JSON.parse(cashflows) : cashflows;
    if (!Array.isArray(flows) || flows.length < 2) {
      return { success: false, error: '现金流至少需要 2 期' };
 ...
 * @returns {Promise<Object>}
 */
async function loanPayment(principal, annualRate, months) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function amortizationSchedule(principal, annualRate, months) {
  try {
    principal = toNum(principal); annualRate = toNum(annualRate); months = toNum(months);
    if (!principal || principal <= 0) return { success: false, error: '本金必须为正数' };
 ...
 * @returns {Promise<Object>}
 */
async function totalInterest(principal, annualRate, months) {
  try {
 ...
 * @returns {Promise<Object>}
 */
async function movingAverage(data, period) {
  try {
    period = toNum(period);
 ...
 * @returns {Promise<Object>}
 */
async function volatility(prices) {
  try {

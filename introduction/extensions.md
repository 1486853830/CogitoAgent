...
MCP（Model Context Protocol）支持将工具暴露为 JSON-RPC 服务，供其他 AI 客户端调用。
...
MCP 服务随 Agent 启动自动启用，默认监听端口配置在 `config.json` 中。
...
> 本文档涵盖了插件系统、MCP 协议、追踪模块、重试机制、多模型支持和联网搜索六大扩展功能的详细说明。